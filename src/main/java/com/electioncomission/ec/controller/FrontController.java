...
import com.electioncomission.ec.config.CustomUsersDetails;
...
public class FrontController {
    ...
    @PostMapping("/login")
    public String doLogin(HttpServletRequest request, @Valid SmsPojo smsPojo, BindingResult bindingResult, Model model, RedirectAttributes redirectAttrs)
    {
        if (bindingResult.hasErrors()) {
            model.addAttribute("error","");
            bindingResult.getAllErrors().forEach(e -> {
                model.addAttribute("error",model.getAttribute("error")+e.getDefaultMessage());
                System.out.println("ERROR  = "+e.toString());
            });
            model.addAttribute("mobileNumber", smsPojo.getMobileNumber());
            return "basic/login";
        }
        redirectAttrs.addFlashAttribute("mobileNumber", smsPojo.getMobileNumber());
        return "redirect:/otp";
